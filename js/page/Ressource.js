...
class PageRessource
{
	...
	*/
	saveChasse(listeChasse)
	{
        ...
        if(!dataEvo.hasOwnProperty("chasse") || dataEvo.chasse.length != listeChasse.length || dataEvo.chasse[0]["quantite"] != listeChasse[0]["quantite"] || listeChasse[0]["exp"].diff(dataEvo.chasse[0]["exp"], 's') > 1){
            dataEvo.chasse = listeChasse;
            dataEvo.startChasse = moment();
            localStorage.setItem("outiiil_data", JSON.stringify(dataEvo));
            if(!Utils.comptePlus && $("#boiteComptePlus").length){
                this._boiteComptePlus.chasse = dataEvo.chasse;
                this._boiteComptePlus.startChasse = dataEvo.startChase;
                this._boiteComptePlus.majChasse();
            }
        }
	}
}
