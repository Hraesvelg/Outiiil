...
class PageReine
{
	...
	*/
	plus()
	{
		...
		$(".icones_unite").each((i, elt) => {
			...
			$(elt).append(`<table class="tab_stat" style="display: none;"><tbody><tr><td style="text-align:center;font-size:0.8em;height:30px;" colspan="2"> Avec Bonus</td></tr><tr title="Vie avec Bouclier niveau ${monProfil.niveauRecherche[1]}"><td class="icone_vie" style="position:relative; top:4px"><img width="19" height="19" src="images/icone/icone_coeur.gif" alt='Vie :'/></td><td class="vie" style="white-space:nowrap">${(VIE_UNITE[index] + (VIE_UNITE[index] / 10 * monProfil.niveauRecherche[1]).toFixed(1)/1)}</td></tr><tr title="Dégats en Attaque avec Armes niveau ${monProfil.niveauRecherche[2]}"><td class="icone_degat_attaque" style="position:relative;top:3px"><img width="18" height="18" src="images/icone/icone_degat_attaque.gif" alt="Dégat en attaque :"/></td><td class="degat_defense" style="white-space:nowrap">${(ATT_UNITE[index] + (ATT_UNITE[index] / 10 * monProfil.niveauRecherche[2]).toFixed(1)/1)}</td></tr><tr title="Dégats en Défense avec Armes niveau ${monProfil.niveauRecherche[2]}"><td class="icone_degat_defense" style="position:relative;top:3px"><img width="18" height="18" src="images/icone/icone_degat_defense.gif" alt="Dégat en défense :"/></td><td class="degat_defense" style="white-space:nowrap">${(DEF_UNITE[index] + (DEF_UNITE[index] / 10 * monProfil.niveauRecherche[2]).toFixed(1)/1)}</td></tr><tr><td style="height:30px;" colspan="2"></td></tr></tbody></table>`);
		});
	...
	*/
	savePonte(listePonte)
	{
        ...
        if(!dataEvo.hasOwnProperty("ponte") || dataEvo.ponte.length != listePonte.length || listePonte[0]["exp"].diff(dataEvo.ponte[0]["exp"], 's') > 1){
            dataEvo.ponte = listePonte;
            dataEvo.startPonte = moment();
            localStorage.setItem("outiiil_data", JSON.stringify(dataEvo));
            if(!Utils.comptePlus && $("#boiteComptePlus").length){
                this._boiteComptePlus.ponte = dataEvo.ponte;
                this._boiteComptePlus.startPonte = dataEvo.startPonte;
                this._boiteComptePlus.majPonte();
            }
        }
	}
}
