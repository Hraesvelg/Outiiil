...
class PageAttaquer
{
        ...
        */
        ...
        if($("#tabChoixArmee").length){
            ...
            this._cible.getProfil().then((data) => {
                this._cible.chargeDataProfil(data);
    ...
    */
    majStatistique(armee = null)
    {
		let tmp = armee ? armee : new Armee({unite : this.extraitArmee()}), html = `<table id="o_tableStatArmee" cellspacing=0>
            ...
            <tr><td><img alt='Vie :' class='o_vAlign' src='images/icone/icone_coeur.gif' height='18' title='Vie' \></td><td>${numeral(tmp.getBaseVie()).format()}</td><td>${numeral(tmp.getTotalVie(monProfil.niveauRecherche[1], 0, 0)).format()}</td></tr>
            <tr><td><img alt='Dégats en attaque :' class='o_vAlign' src='images/icone/icone_degat_attaque.gif' height='18' title='Dégat en attaque :' /></td><td>${numeral(tmp.getBaseAtt()).format()}</td><td>${numeral(tmp.getTotalAtt(monProfil.niveauRecherche[2])).format()}</td></tr>
            <tr><td><img alt='Dégat en défense :' class='o_vAlign' src='images/icone/icone_degat_defense.gif' height='18' title='Dégats en défense :' /></td><td>${numeral(tmp.getBaseDef()).format()}</td><td>${numeral(tmp.getTotalDef(monProfil.niveauRecherche[2])).format()}</td></tr>
    ...
    */
    ajouterOption()
    {
        ...
        $("#o_btnLancer").before(`<div id="o_btnSynchro"><button id='o_synchro' class="o_button f_info">Synchroniser</button><button id='o_sonder' class="o_button f_error">Sonder</button></div>`).after(`<p class="centre reduce ligne_paire">Votre armée rentrera le <span id="o_retourArmee" class="gras">${moment().add(this._cible.getTempsParcours(), 's').format("D MMM à HH[h]mm[m]ss[s]")}</span> (RC : <span id="o_retourArmeeRC" class="gras">${Utils.roundMinute(this._cible.getTempsParcours()).format("D MMM à HH[h]mm")}</span>).</p>`);
        $("#o_synchro").click((e) => {
            e.preventDefault();
        ...
        Utils.incrementTime(this._cible.getTempsParcours(), "o_retourArmee", "o_retourArmeeRC");
    }
    /**
    *
    */
	...
	*/
    formulaireFlood()
    {
        ...
        $(".simulateur:eq(0)").append(`<fieldset id='o_prepaFlood' class='centre'><legend><span class='titre'>Lanceur de Flood</span></legend>
			...
			<tr class='gras'><td>Etape</td><td>Troupes</td><td>Supp.*</td><td>Mon Terrain</td><td>${this._cible.pseudo} (${Utils.intToTime(this._cible.getTempsParcours())})</td></tr>
    ...
    */
    ajouterAttaque()
    {
        ...
        if(nbAttaque >= this._nbAttaque)
            $.toast({...TOAST_WARNING, text : "Votre vitesse d'attaque ne vous permez d'envoyer plus d'attaques"});
        else{
