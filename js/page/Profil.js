...
class PageProfil
{
    ...
    */
    initialise()
    {
        ...
        if(!this._profil.estJoueurCourant()){
            ...
            $(".boite_membre:first div:first table").append(`<tr><td class='right'>Retour le :</td><td id='o_tempsRetour'>${moment().add(this._profil.getTempsParcours(), 's').format("D MMM à HH[h]mm[m]ss[s]")}</td></tr><tr><td class='right'>Rapport :</td><td id='o_tempsRetourRapport'>${Utils.roundMinute(this._profil.getTempsParcours()).format("D MMM à HH[h]mm")}</td></tr>`);
            Utils.incrementTime(this._profil.getTempsParcours(), "o_tempsRetour", "o_tempsRetourRapport");
        }
    ...
    */
    plus()
    {
        ...
        $(".boite_membre:first div:first table").append(`<tr><td style='text-align:right'>Temps de trajet :</td><td>${Utils.intToTime(this._profil.getTempsParcours())}</td></tr>`);
    }
}
