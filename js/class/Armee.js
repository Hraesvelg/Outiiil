...
class Armee
{
	...
	*/
	...
	{
		let s = "";
        this._unite.every((elt, ind) => {return s += (elt ? numeral(elt).format() + " " + NOM_UNITE[ind + 1] + ", " : "");});
		return s.slice(0, -2) + ".";
	}
    /**
	...
	*/
	parseArmee(str)
	{
		...
		for(let i = 0 ; i < tmp.length ; this._unite[NOM_UNITE.indexOf((tmp[i] + " ").replace(/[0-9]/g, '').replace(/s /g, ' ').trim()) - 1] = parseInt(tmp[i++].replace(/\D+/g, '')));
	}
	/**
	...
	*/
	getSommeUnite()
	{
        return this._unite.reduce((acc, val) => {return acc + val;}, 0);
	}
	/**
	...
	*/
	getTotalVie(bonus, lieu, bonusLieu)
	{
...
//                donnees["unite11"] = this._repartition[indice][12];
//                donnees["unite12"] = this._repartition[indice][13];
//                donnees["unite13"] = this._repartition[indice][6];
//                donnees["unite14"] = this._repartition[indice][11];
