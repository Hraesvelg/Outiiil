...
class Alliance
{
    constructor(parametres)
    {
        ...
        *
        */
        this._rangs = new Array();
        if(parametres.hasOwnProperty("rangs"))
            for(let i = 0 ; i < parametres["rangs"].length ; i++)
                this._rangs.push(new Rang(parametres["rangs"][i].libelle, parametres["rangs"][i].ordre));
        /**
        *
        */
    ...
    */
    toJSON()
    {
        return {tag : this._tag, joueurs : this._joueurs, terrain : this._terrain, technologie : this._technologie, fourmiliere : this._fourmiliere, rangs : this._rangs, sujetForum : this._sujetForum};
    }
    /**
    *
    */
    chercheRang(libelle)
    {
        return this._rangs.find((elt, ind) => {return elt.libelle == libelle;});
    }
    /**
    *
    */
    ajouteRang(rang)
    {
        if(!this.chercheRang(rang.libelle)) this._rangs.push(rang);
    }
    /**
    ...
    */
    getHistorique(id)
    {
		...
		$.get("http://outiiil.fr/fzzz/team/" + Utils.serveur + "/" + this._tag, (data) => {
    ...
    */
    ...
    {
        $("#" + id).append(`<tr class="lien"><td><a id="o_maj_${this._tag}" class='o_actualiser' href=""><img src="${O_ACTUALISER}" alt="rang" height="20"/></a></td><td class="left"><a class="gras" href="classementAlliance.php?alliance=${this._tag}">${this._tag}</a></td><td id="o_terrain_${this._tag}" class="right reduce" title="">${numeral(this._terrain).format()}</td></tr>`);
        // event
