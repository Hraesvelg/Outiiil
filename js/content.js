...
!function()
{
	...
	if ($(".boite_connexion_titre:first").text() != "Connexion"){
        ...
        monProfil = new Joueur({pseudo : Utils.pseudo});
        ...
        $.when(monProfil.getConstruction(), monProfil.getLaboratoire(), monProfil.getProfilCourant()).then((dataConstruction, dataRecherche, dataProfil) => {
            // chargement des données du joueur
            if(dataConstruction) monProfil.chargeDataConstruction(dataConstruction[0]);
            if(dataRecherche) monProfil.chargeDataLaboratoire(dataRecherche[0]);
            if(dataProfil) monProfil.chargeDataProfil(dataProfil[0]);
            ...
            // boite compte plus
            instanceBCP = null;
            let boiteComptePlus = new BoiteComptePlus(JSON.parse(localStorage.getItem("outiiil_data")) || {});
